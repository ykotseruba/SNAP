...
# Provide full path SNAP code directory
CODE_FOLDER=$SNAP_PATH

# Provide full path to SNAP image data
DATA=$SNAP_DATA_PATH
...
TAG=torch2.0
CONTAINER_NAME=torch_sensor_bias
...
docker run --rm -it --gpus \"device=${GPU_DEVICE}\"  \
	...
	--mount type=bind,source=${DATA},target=${WORKING_DIR}/data \
	-e DATA=${WORKING_DIR}/data \
	-m ${MEMORY_LIMIT} \
	-w ${WORKING_DIR} \
	-e log=/home/log.txt \
	-e HOST_UID=$(id -u) \
